class PurchaseCostItems < ActiveRecord::Base
  ...
  def self.get_purchase_cost(gameboard_id,segment,category, seller_decl)
    sql = "SELECT cost FROM purchase_cost_items pi INNER JOIN purchase_order_header ph ON ph.id = pi.purchase_cost_header_id AND ph.segment = '#{segment}'" +
          " INNER JOIN cost_sheet cs ON ph.cost_sheet_id = cs.id and cs.gameboard_id = '#{gameboard_id}' WHERE " +
